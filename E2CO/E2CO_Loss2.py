...
def loss_sql2norm(x_true, x_predict):
    output = K.mean(K.sum((K.batch_flatten(x_true) - K.batch_flatten(x_predict)) ** 2, axis=-1))
    return output


def loss_l2norm(x_true, x_predict):
    output = K.mean(K.sqrt(K.sum((K.batch_flatten(x_true) - K.batch_flatten(x_predict)) ** 2, axis=-1)))
    return output
...
class e2co_loss(Loss):
    ...
    def call(self, true, prediction):
        xt, zt1, xt1, yt1 = true
        xt_rec, zt1_predict, xt1_predict, yt1_predict = prediction

        self.rec_loss = loss_l2norm(xt, xt_rec)
        self.pred_loss = loss_l2norm(xt1, xt1_predict)
        self.trans_loss = loss_l2norm(zt1, zt1_predict)
        ...
        flux_pred_loss = loss_flux(xt1, xt1_predict, SWATTableFile=self.SWATTableFile, GRIDMinMax=self.GRIDMinMax, NumberOfRealizations=self.NE,
                                  PermFolder=self.PermFolder, Nx=self.Nx, Ny=self.Ny, Nz=self.Nz)
        self.total_flux_loss = (flux_rec_loss + flux_pred_loss)

        self.well_loss = loss_l2norm(yt1, yt1_predict)

        self.total_loss = (self.rec_loss + self.pred_loss) * self.weight_loss_rec_pred + self.trans_loss * self.weight_loss_trans \
                          + self.total_flux_loss * self.weight_loss_flux + self.well_loss * self.weight_loss_well
