...
class GTrXLNet(RecurrentNetwork):
    ...
    def __init__(self,
        ...
        ## Decide whether incoming observation data is for training or evaluation.

        time_step = 11
        y = math.ceil(self.obs_dim/time_step)
        ## Preprocess observation with CNN
        x = tf.keras.layers.Reshape((-1,time_step,y))(input_layer)  ## time_step = the num of time steps between control steps (num_run_per_control_step)
        x = tf.keras.layers.Conv1D(64, 2, activation='relu', input_shape=(time_step, y))(x) 
        x = tf.keras.layers.Conv1D(64, 3, activation='relu')(x) 
        x = tf.keras.layers.Reshape((-1,x.shape[-1]*x.shape[-2]))(x)   #(-1,64)
        ...
        for i in range(self.num_transformer_units):
            ...
            E_out = SkipConnection(
                #Layer Norm and Position-wise MLP
                tf.keras.Sequential(
    ...
    def forward(self, input_dict, state: List[TensorType],
       ...
       # observations = input_dict["obs_flat"]
        ...
        #print('OBSERVATION DIMENSION before reshape: ',shape)
        T = shape[0] // B
        observations = tf.reshape(observations, tf.concat([[-1, T], shape[1:]], axis=0))
        #print('OBSERVATION after reshaping: ',  observations)
        all_out = self.trxl_model([observations] + state)

        if self._logits is not None:
            out = tf.reshape(all_out[0], [-1, self.num_outputs])  #mean and std of well controls
            self._value_out = all_out[1]  #value function
    ...
    # @override(RecurrentNetwork)
    # def get_initial_state(self) -> List[np.ndarray]:
    #     return  [np.zeros(self.obs_dim, np.float32)] * self.max_seq_len #[np.zeros((self.max_seq_len, self.obs_dim), np.float32)]
    ...
    def get_initial_state(self) -> List[np.ndarray]:
        initial_states = []
        for _ in range(self.num_transformer_units):
            initial_states.append(np.zeros((self.attention_dim), np.float32))
...
class GTrXLNet2(RecurrentNetwork):
    """A GTrXL net Model described in [2].
    This is still in an experimental phase.
    Can be used as a drop-in replacement for LSTMs in PPO and IMPALA.
    For an example script, see: `ray/rllib/examples/attention_net.py`.
    ...
    def __init__(self,
        ...
        # Preprocess observation with CNN
        x = tf.keras.layers.Conv1D(64, 2, activation='relu', name="conv1")(input_layer)
        x = tf.keras.layers.Conv1D(64, 3, activation='relu', name="conv2")(x)
        
        # Map observation dim to input/output transformer (attention) dim.
        E_out = tf.keras.layers.Dense(self.attention_dim, name="dense1")(x)
        # Output, collected and concat'd to build the internal, tau-len
        # Memory units used for additional contextual information.
        memory_outs = [E_out]
